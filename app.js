(() => {
  ...
  function saveSettings(){const key=$('geminiKey').value.trim(),model=$('geminiModel').value.trim()||'gemini-2.0-flash';if(key)localStorage.setItem(storage.key,key);else localStorage.removeItem(storage.key);localStorage.setItem(storage.model,model);closeSettings();toast('Settings saved.');}
